...
BEGIN
	...
	-- LOADING silver.sale_detail
	SET @batch_start_time = GETDATE();
	PRINT '>> TRUNCATING TABLE : silver.sale_detail';
	TRUNCATE TABLE silver.sale_detail;
	PRINT '>> INSERTING DATA INTO : silver.sale_detail';
	INSERT INTO silver.sale_detail (
			sls_ord_num,
			sls_prd_key,
			sls_cust_id,
			sls_order_dt,
	...
	PRINT 'LOAD DURATION FOR SILVER LAYER IS :' + CAST(DATEDIFF(second,@batch_start_time,@batch_end_time) AS NVARCHAR) + 'seconds';
	END TRY
